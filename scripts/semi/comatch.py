    ...
    if pm.dataset in {'cifar10', 'cifar100', 'cifar100-big'}:
        for seed in range(pm.seeds):
            for n_percls in [4, 25, 100]:
                yield dict(n_percls=n_percls, scan=f'comatch-{DATE_FLAG}', seed=seed)
    elif pm.dataset == 'stl10':
        for seed in range(pm.seeds):
            yield dict(scan=f'comatch-{DATE_FLAG}', seed=seed, n_percls=100)
...
configs = {
    'cifar10': 'config/semi/comatch/cifar10.yml',
    'cifar100': 'config/semi/comatch/cifar100.yml',
    'cifar100-big': 'config/semi/comatch/cifar100-big.yml',
    'stl10': 'config/semi/comatch/stl10.yml',
}
...
def main():
    ...
    for kwargs in mapping[pm.type](pm):
        ...
        dics.append(dict(module='comatch', config=configs[pm.dataset], **kwargs))
