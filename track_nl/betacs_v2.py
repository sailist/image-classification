...
import torch
from lumo.contrib.nn.functional import batch_cosine_similarity2, sharpen
...
from lumo.contrib.nn.loss import cross_entropy_with_targets, contrastive_loss2
...
class BetaGLTrainer(NLTrainer):
    ...
    def imodels(self, params: ParamsType):
        ...
        # meter
        self.mb = MemoryBank()
        self.mb.register('pred', params.n_classes, params.K)
        self.mb.register('feature', params.feature_dim, params.K)
    ...
    def train_step(self, batch, params: ParamsType = None) -> MetricType:
        ...
        w_feature = w_outputs.feature

        outputs = self.model.forward(axs)
        logits = outputs.logits
        a_feature = outputs.feature

        b_outputs = self.model.forward(bxs)
        b_feature = b_outputs.feature

        # filter
        sim = batch_cosine_similarity2(w_feature, self.mb['feature'], 1e-8)
        values, indices = sim.topk(10, dim=-1)
        mb_p_label = self.mb['pred'][indices].argmax(dim=-1)
        eq_cnt = torch.eq(nys.unsqueeze(1), mb_p_label).sum(dim=-1)
        neighbor_mask = eq_cnt > 5

        un_w_pys = torch.argmax(w_logits, dim=-1)
        qk_graph = torch.eq(un_w_pys[:, None], un_w_pys[None, :])

        with torch.no_grad():
            meter.mean.nem = neighbor_mask.float().mean()

        qk_graph[neighbor_mask, :] = 0
        qk_graph[:, neighbor_mask] = 0

        qk_graph = qk_graph | torch.eye(len(qk_graph), device=self.device, dtype=torch.bool)
        qk_graph = sharpen(qk_graph, 1)

        Lcs = contrastive_loss2(a_feature, b_feature, qk_graph=qk_graph,
                                norm=True, temperature=0.1)

        with torch.no_grad():
            self.mb.push('feature', w_feature)
            self.mb.push('pred', torch.softmax(w_logits, dim=-1))
        ...
        Lpce = (cross_entropy_with_targets(logits, p_targets, (1 - fweight) * values * mask)
